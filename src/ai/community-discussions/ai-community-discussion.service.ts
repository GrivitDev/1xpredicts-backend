...
export class AiCommunityDiscussionService {
  ...
  async generateDiscussion(
    ...
    const prompt = this.buildPrompt(selectedPrediction, context);
    ...
    return this.validateDiscussion(result.data);
  }
...
Use Google Search to investigate this match.
...
For example, question whether the prediction is justified,
support the prediction with current evidence, or challenge
it based on a meaningful football factor.
...
Do not mention that the discussion was generated by AI.
...
When a 2xPredict prediction is supplied, make the discussion
directly relevant to that prediction.
...
The goal is to encourage football fans to disagree,
agree, or explain their own view.
`.trim();
  }
