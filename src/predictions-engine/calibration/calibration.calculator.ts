...
export class CalibrationCalculator {
  calculate(input: {
     ...
     * Calibration should correct a systematic bias,
     * not overpower the underlying model.
     */
    const rawAdjustment = actualSuccessRate - averageProbability;

    const adjustment =
      sampleSize >= 20 ? this.clamp(rawAdjustment * 0.35, -0.1, 0.1) : 0;

    const reliabilityScore = this.clamp(
      (sampleReliability * 0.45 + accuracyReliability * 0.55) * 100,
      0,
      100,
    );
